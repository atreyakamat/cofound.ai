...
export default function DecisionPage() {
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      ...
      setDecision((prev) =>
        prev
          ? { ...prev, messages: prev.messages.filter((m) => m.id !== "temp") }
          : null
      );
    } finally {
      setSending(false);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {decision.category} • {decision.status.replace("_", " ")}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4 shadow-xl">
            ...
            <form onSubmit={saveOutcome} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  How would you rate the outcome? (1-5)
                </label>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowOutcome(false)} className="btn-secondary flex-1">
                  Cancel
                </button>
      ...
      {decision.outcome && (
        <div className="border-t border-gray-200 bg-green-50 px-6 py-4">
          ...
          <p className="text-xs text-green-600 mt-1">
            Rating: {"⭐".repeat(decision.outcomeRating || 0)}
          </p>
        </div>
      )}
    </div>
