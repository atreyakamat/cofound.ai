...
// ...existing code (Message, Decision interfaces stay the same)...
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="border-b border-gray-200 bg-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {(decision.status === "decided" || decision.status === "tracking") && !decision.outcome && (
            <button onClick={() => setShowOutcome(true)} className="btn-secondary text-xs">
              📝 Log Outcome
            </button>
          )}
        </div>
      ...
      {decision.status !== "completed" && (
        <div className="border-t border-gray-200 bg-white px-6 py-4">
          {interimText && (
            <p className="text-xs text-gray-400 italic mb-2">🎤 {interimText}</p>
          )}
